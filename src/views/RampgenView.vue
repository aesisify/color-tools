	...
	<section class="p-6">
		<div>
			<div class="flex justify-center bg-gray-900 p-2 rounded">
				<div @click="copyToClipboard(color)"
					class="group flex relative max-h-16 flex-grow items-center justify-center aspect-square cursor-pointer"
					v-for="color in colorRamp" :style="{ background: color }">
					<div class="hidden group-hover:flex absolute top-2 right-2 bg-gray-800 bg-opacity-50 rounded p-0.5">
						<Icon icon="ph:copy"></Icon>
					</div>
				</div>
			</div>
			<color-picker class="bg-gray-900 p-2 rounded mt-3" v-model="middleColor"></color-picker>
			<div class="mt-3 bg-gray-900 p-2 rounded">
				<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
					<div>
						<div class="text-sm uppercase tracking-widest border-b px-2 py-1 border-b-indigo-500">
							General Settings
						</div>
						<div class="p-2 flex flex-col gap-2">
							<div class="flex flex-col">
								...
								<input class="bg-gray-800 p-1 px-2 rounded slider" v-model="generalSettings.colorCount"
									max="24" min="3" step="1" type="range" />
							</div>
							<div class="flex flex-col">
								<label class="text-xs font-medium mb-1">
								...
								<select class="bg-gray-800 p-1 px-2 rounded" v-model="generalSettings.interpolation">
							...
							<label class="font-medium text-xs select-none">
								<input type="checkbox"
									...
									v-model="generalSettings.correctLightness" checked />
								Correct Lightness
							</label>
						</div>
					</div>
					<div>
						...
						<div class="p-2 flex flex-col gap-2">
							<div class="flex flex-col">
								...
								<select class="bg-gray-800 p-1 px-2 rounded" v-model="startColorSettings.mode">
								...
								<div class="flex flex-col">
									<label class="text-xs font-medium mb-1">
										<div class="flex items-center justify-start gap-1">
											...
											<div :style="{ background: hueShift.start }" class="h-3 w-3 rounded"></div>
										</div>
									</label>
									<input class="bg-gray-800 p-1 px-2 rounded slider"
										v-model="startColorSettings.hueShift" max="100" min="-100" step="0.1"
										type="range" />
								</div>
								<div class="flex flex-col">
									<label class="text-xs font-medium mb-1">
									...
									<input class="bg-gray-800 p-1 px-2 rounded slider"
										v-model="startColorSettings.brightness" min="-5" max="5" step="0.1"
										type="range" />
								</div>
								<div class="flex flex-col">
									<label class="text-xs font-medium mb-1">
									...
									<input class="bg-gray-800 p-1 px-2 rounded slider"
										v-model="startColorSettings.saturation" min="-3" max="3" step="0.1"
										type="range" />
								</div>
							</template>
							<template v-else>
								<color-picker class="bg-gray-700 p-2 rounded mt-1"
					...
					<div>
						...
						<div class="p-2 flex flex-col gap-2">
							<div class="flex flex-col">
								...
								<select class="bg-gray-800 p-1 px-2 rounded" v-model="endColorSettings.mode">
									<option value="generate">Hue Shift</option>
									<option value="select">Select Manually</option>
								</select>
							</div>
								...
								<div class="flex flex-col">
									<label class="text-xs font-medium mb-1">
										<div class="flex items-center justify-start gap-1">
											...
											<div :style="{ background: hueShift.end }" class="h-3 w-3 rounded"></div>
										</div>
									</label>
									<input class="bg-gray-800 p-1 px-2 rounded slider"
										v-model="endColorSettings.hueShift" max="100" min="-100" step="0.1"
										type="range" />
								</div>
								<div class="flex flex-col">
									<label class="text-xs font-medium mb-1">
									...
									<input class="bg-gray-800 p-1 px-2 rounded slider"
										v-model="endColorSettings.brightness" min="-5" max="5" step="0.1"
										type="range" />
								</div>
								<div class="flex flex-col">
									<label class="text-xs font-medium mb-1">
									...
									<input class="bg-gray-800 p-1 px-2 rounded slider"
										v-model="endColorSettings.saturation" min="-3" max="3" step="0.1"
										type="range" />
								</div>
							</template>
							<template v-else>
					...
					<div class="col-span-1 md:col-span-3">
						...
						<div class="p-2 flex flex-col gap-2">
							...
							<div class="text-xs">
								Palette DeltaE:
								<span :class="{ 'text-red-500': accesability.paletteDeltaE < 25 }"
									v-text="accesability.paletteDeltaE.toFixed(2)"></span>
							</div>
							<div class="text-xs">
								Lowest DeltaE:
								<span :class="{ 'text-red-500': accesability.lowestDeltaE < 2.5 }"
			...
			<div class="mt-3 bg-gray-900 p-2 rounded" style="white-space: pre-wrap">
				<div class="text-sm uppercase tracking-widest border-b px-2 py-1 border-b-indigo-500">Tailwind Output</div>
				<div class="font-mono mt-2 text-sm px-3 py-2 bg-gray-800 rounded"
					v-html="'&quot;color-name&quot;: ' + removeQuotesFromJson(JSON.stringify(outJson, null, '\t'))">
				</div>
			</div>
		</div>
	</section>
